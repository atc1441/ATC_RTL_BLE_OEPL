 ...
 * If the "len" field sets to the size of a "uint16_t", the "p_value" field must point to a data with type "uint16".
 *
 ...
 * @brief   Start a device discovery extended scan.
            If the device changes to the scanning state, APP processing function will be called. And the
            advertising data or scan response data will be returned by the callback function with
            msg type @ref GAP_MSG_LE_EXT_ADV_REPORT_INFO.
 *
 * @return  Operation result.
...
/**
  * @brief   Stop a device discovery extended scan.
             If device changes to idle state, APP processing function will be called.
  *
  * @return  Operation result.
...
*         APP. Message type is @ref GAP_MSG_LE_EXT_SCAN_STATE_CHANGE_INFO.
*
