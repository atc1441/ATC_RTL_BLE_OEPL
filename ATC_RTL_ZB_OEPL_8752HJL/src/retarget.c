 ...
 * -------------------------------------------------------------------------- */
/* Wait until both the TX FIFO and shift register are empty (TEMT = LSR bit 6).
 * Must be called before entering DLPS — platform PM blocks sleep if UART is busy. */
void uart_flush(void)
{
    volatile uint32_t t = 200000;
    while ((UART0->LSR & (1u << 6)) == 0 && --t)
        ;
}
