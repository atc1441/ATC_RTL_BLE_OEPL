...
bool mac_edit_update(uint8_t *new_mac)
{
    ...
    T_IMG_CTRL_HEADER_FORMAT *p_hdr = (T_IMG_CTRL_HEADER_FORMAT *)buffer;

    // Validate header
    if (p_hdr->image_id != IMAGE_FIRST) {
        printf("MAC EDIT: Invalid image ID 0x%04X (expected 0x%04X)\n",
               p_hdr->image_id, IMAGE_FIRST);
        os_mem_free(buffer);
        return false;
    }
    ...
    // Update SHA256
    uint32_t payload_len = p_hdr->payload_len;
	...
	// HEADER HAS UNKNOWN HASH ALGO BUT THE ROM DOES NOT CARE ANYWAY SO ITS LEFT OUT
    /*uint32_t hash[8];
    hw_sha256_init();
    // Mode 0 is HW_SHA256_CPU_MODE
    if (hw_sha256(&buffer[0x1B0], payload_len + 0x250, hash, 0)) {
        memcpy(p_hdr.image_hash, hash, 32);
