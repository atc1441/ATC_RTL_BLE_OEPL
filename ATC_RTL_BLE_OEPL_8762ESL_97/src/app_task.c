...
void app_main_task(void *p_param)
{
    ...
    /* Keep the original 5.85" AON watchdog architecture, but use a 60 s
     * timeout for extra margin.  Normal operation feeds it on every DLPS exit
     * (main.c).  The 9.7" display path additionally feeds it immediately before
     * starting a long physical EPD refresh, during which DLPS is intentionally
     * blocked by the v7 refresh guard. */
    aon_wdg_init(AON_WDT_RESET_ALL, AON_WDG_TIMEOUT_S);
    aon_wdg_enable();
    printf("AON watchdog enabled: %u s\n", (unsigned)AON_WDG_TIMEOUT_S);
