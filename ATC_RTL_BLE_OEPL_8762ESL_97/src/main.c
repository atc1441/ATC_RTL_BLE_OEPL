...
/**
 * @brief    DLPS admission check.
 *
 * The 9.7" EPD controller must remain fully powered and its GPIO state must
 * remain untouched for the complete ~24 s physical refresh.  The asynchronous
 * EPD poll timer continues to run, but DLPS itself is vetoed while
 * g_epd_refreshing is true.  As soon as epd_refresh_done() clears the flag,
 * normal DLPS operation is allowed again.
 */
#if F_BT_DLPS_EN
static bool app_dlps_check_cb(void)
{
    return g_epd_refreshing ? false : true;
}
#endif
...
{
#if F_BT_DLPS_EN
    /* Ask the power manager before every DLPS entry.  A running EPD refresh
     * returns false here, so the CPU stays active until BUSY falls LOW and
     * epd_refresh_done() clears g_epd_refreshing. */
