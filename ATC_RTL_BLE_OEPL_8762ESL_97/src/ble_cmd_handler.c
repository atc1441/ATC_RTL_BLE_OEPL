...
/* EL097R2CRN RAW 2bpp = 2 * 80640 = 161280 bytes (+ header).
 * 0x28000 = 163840 bytes is the smallest 4 KiB-aligned slot that fits it.
 * With OTA_EXT_START at 0x45000 this intentionally leaves one image slot. */
#define DEFAULT_IMG_EACH 0x28000UL
...
uint8_t imgSlots = 0; /* computed from OTA_EXT_START / img_each() */
...
static void recalcParts(void)
{
    ...
    partsThisBlockBLE = (uint8_t)((sizeof(struct blockData) + dataRequestSizeBLE + BLOCK_PART_DATA_SIZE_BLE - 1) / BLOCK_PART_DATA_SIZE_BLE);
}
...
bool processAvailDataInfoBLE(struct AvailDataInfo *avail)
{
    ...
    if (imgSlots > OTA_IMG_SLOTS_MAX)
        imgSlots = OTA_IMG_SLOTS_MAX;
    if (imgSlots == 0)
        imgSlots = 1;
