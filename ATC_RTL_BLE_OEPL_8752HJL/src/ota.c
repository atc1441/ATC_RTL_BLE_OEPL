 ...
 *      normal code; this is safe because OTA_TMP != APP area (0x836000).
...
void ota_apply(uint32_t fw_size)
{
     ...
     * prepend_header.exe).  T_IMG_HEADER_FORMAT is its first 12 bytes and
    ...
    T_IMG_HEADER_FORMAT *p_hdr =
        (T_IMG_HEADER_FORMAT *)OTA_TMP_ADDR;

    printf("OTA: validate id=0x%04X payload_len=%lu crc16=0x%04X\n",
           p_hdr->ctrl_header.image_id,
           (unsigned long)p_hdr->ctrl_header.payload_len,
