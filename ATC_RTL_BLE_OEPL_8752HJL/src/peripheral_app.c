...
void set_adv_data(uint16_t battery_mv)
{
    ...
    ble_adv_data.temperature = temperature_measure_celsius();
    ...
    le_adv_set_param(GAP_PARAM_ADV_DATA, sizeof(struct BleAdvDataStruct), (void *)&ble_adv_data); // adv_len, adv_data_buf);

    /* 3. Set Scan Response Data: OEPL Struct only */
    le_adv_set_param(GAP_PARAM_SCAN_RSP_DATA, adv_len, adv_data_buf);
