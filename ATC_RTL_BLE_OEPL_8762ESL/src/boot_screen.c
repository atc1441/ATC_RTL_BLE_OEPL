...
/* In X-decrement mode the SSD1619 maps bit 7 → highest X (rightmost pixel) of
 * each byte's group, but the font stores bit 7 = leftmost pixel.
 * Bit-reversing each byte corrects the horizontal flip within every glyph. */
 ...
 * Returns 0xFF (all white) for out-of-range inputs. */
static inline uint8_t char_byte(char c, int fr)
{
    if (fr < 0 || fr > 7 || c < 0x20 || c > 0x7E)
        return 0xFF;
    return ~bit_rev8(font8[(uint8_t)c - 0x20][fr]);
}

/* ---- Layout constants ---------------------------------------------------- */
#define ROW_BYTES   100   /* bytes per full-width row sent to both panels       */
#define CHAR_H      20    /* text row height: 8-pt font × 2× (16 px) + 4 px gap */
#define EPH         272   /* display height in pixels                           */
...
static void render_lines(const char * const *lines, int n_lines)
{
    ...
    for (int y = 0; y < EPH; y++) {
        ...
        epd_cmd(0xA4); epd_stream_data(row,      50);
        epd_cmd(0x24); epd_stream_data(row + 49, 50);
    }
